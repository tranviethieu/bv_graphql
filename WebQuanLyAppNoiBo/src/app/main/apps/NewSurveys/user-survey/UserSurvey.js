...
import * as Actions from '../surveys/store/actions';
...
function UserSurvey(props) {
    ...
    function fetchData() {
        ...
        // dispatch(showMessage({ message: `survey id: ${surveyId}` }))
        if (surveyId) {
            setResult('surveyId', surveyId)
            Actions.getSurvey(surveyId, dispatch).then(response => {
                setForm(response.data)

                // dispatch(showMessage({ message: `survey response: ${response.data}` }))
            });
        }
    }
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        //do link được chuyển hướng từ app nội bộ nên cho phép setup lại token tại đây
        if (token) {
    ...
    function handleSubmit() {
        ...
        if (checkResult() === true) {
            ...
            setLoading(true)
            Actions.submitSurveyResultEmployee(mResult, dispatch)
                .then((response) => {
                    setLoading(false)
