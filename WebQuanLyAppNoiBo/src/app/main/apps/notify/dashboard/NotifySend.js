...
function NotifySend(props) {
    ...
    function fetchData(state) {
        if (state) {
            ...
            let filter = [...filtered, { id: "appId", value: `${process.env.REACT_APP_CLIENT_ID}` },
            ...
            let filter = [{ id: "appId", value: `${process.env.REACT_APP_CLIENT_ID}` }, { id: 'createdTime', value: `${moment(begin).format("DD/MM/YYYY")},${moment(end).add(1, 'days').format("DD/MM/YYYY")}`, operation: "between" }];
            Actions.getNotificationSends({ page, pageSize, filtered: filter, sorted: tempSort }, dispatch).then(response => {
                console.log("===> notifi queues: ", response)
                setNotifies(response);
                setLoading(false)
