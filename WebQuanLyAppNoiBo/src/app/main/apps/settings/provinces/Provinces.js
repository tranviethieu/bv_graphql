...
import { provincesAttributes } from './components/ProvincesAttributes';
    ...
    return (dispatch) => dispatch(showQuickEditDialog(
        {
            ...
            attributes: attributes || provincesAttributes,
            data: data,
            submit: submit
...
function Provinces(props) {
    ...
    const [attributes, setAttributes] = useState(provincesAttributes);
    ...
    function refetchData() {
        ...
        Actions.get_provinces(merged, dispatch).then(response => {
            setData(response);
        })
    }
    useEffect(() => {
    ...
    function handleSave(form) {
        Actions.save_province(form, dispatch).then(response => {
            if (response.code === 0) {
    ...
    function handleDelete(data) {
        dispatch(showConfirmDialog({
            title: "Xóa Tỉnh/Thành phố", message: `Bạn có chắc muốn xóa Tỉnh/Thành phố ${data.name}?`, onSubmit: () => {
                Actions.remove_province(data._id, dispatch).then(response => {
                    if (response.code === 0) {
                        dispatch(showMessage({ message: "Xóa Tỉnh/Thành phố thành công" }));
                        refetchData();
                    } else {
                        dispatch(showMessage({ message: response.message}));
                    }
              ...
              <div className="flex flex-1 w-full items-center justify-between el-HeaderPageCustom">
                <div className="flex flex-col">
                  <div className="flex items-center mb-4">
                    ...
                    <Icon className="text-16" color="action">chevron_right</Icon>
                    <Typography color="textSecondary">Tỉnh/Thành phố</Typography>
                  </div>
                  <FuseAnimate>
                    <Typography variant="h6">{data.records} Tỉnh/Thành phố</Typography>
                  </FuseAnimate>
                </div>
                <DemoFilter
