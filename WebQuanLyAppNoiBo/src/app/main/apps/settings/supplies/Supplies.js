...
import { suppliesAttributes } from './components/SuppliesAttributes';
    ...
    return (dispatch) => dispatch(showQuickEditDialog(
        {
            ...
            attributes: attributes || suppliesAttributes,
            data: data,
            submit: submit
...
function Supplies(props) {
    ...
    const [attributes, setAttributes] = useState(suppliesAttributes);
    ...
    function refetchData() {
        ...
        Actions.get_supplies(merged, dispatch).then(response => {
            setData(response);
        })
    }
    useEffect(() => {
    ...
    function handleSave(form) {
        Actions.save_supply(form, dispatch).then(response => {
            if (response.code === 0) {
    ...
    useEffect(() => {
        const groupAtt = suppliesAttributes.find(a => a.name === "categoryCode");
        groupAtt.options = categories;
        setAttributes(suppliesAttributes);
    }, [categories]);
    function handleDelete(data) {
        dispatch(showConfirmDialog({
            title: "Xóa Thuốc/Vật tư", message: `Bạn có chắc muốn xóa Thuốc/Vật tư ${data.name}?`, onSubmit: () => {
                Actions.remove_supply(data._id, dispatch).then(response => {
                    if (response.code === 0) {
                        dispatch(showMessage({ message: "Xóa Thuốc/Vật tư thành công" }));
                        refetchData();
                    } else {
                        dispatch(showMessage({ message: response.message}));
                    }
              ...
              <div className="flex flex-1 w-full items-center justify-between el-HeaderPageCustom">
                <div className="flex flex-col">
                  <div className="flex items-center mb-4">
                    ...
                    <Icon className="text-16" color="action">chevron_right</Icon>
                    <Typography color="textSecondary">Thuốc/Vật tư</Typography>
                  </div>
                  <FuseAnimate>
                    <Typography variant="h6">{data.records} Thuốc/Vật tư</Typography>
                  </FuseAnimate>
                </div>
                <DemoFilter
            ...
            content={
              <ReactTable
                ...
                columns={[
                  ...
                  {
                    Header: 'Loại Thuốc/Vật tư/Dịch vụ',
