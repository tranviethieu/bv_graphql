...
import { supplyCategoriesAttributes } from './components/SupplyCategoriesAttributes';
    ...
    return (dispatch) => dispatch(showQuickEditDialog(
        {
            ...
            subtitle: "Loại thuốc/vật tư",
            attributes: attributes || supplyCategoriesAttributes,
            data: data,
            submit: submit
...
function SupplyCategories(props) {
    const dispatch = useDispatch();
    const [data, setData] = useState({});
    const [attributes, setAttributes] = useState(supplyCategoriesAttributes);
    ...
    function refetchData() {
        ...
        Actions.get_supply_categories(merged, dispatch).then(response => {
            setData(response);
        })
    }
    useEffect(() => {
    ...
    function handleSave(form) {
        Actions.save_supply_category(form, dispatch).then(response => {
            if (response.code === 0) {
    ...
    function handleDelete(data) {
        dispatch(showConfirmDialog({
            title: "Xóa Loại thuốc/vật tư", message: `Bạn có chắc muốn xóa Loại thuốc/vật tư ${data.name}?`, onSubmit: () => {
                Actions.remove_supply_category(data._id, dispatch).then(response => {
                    if (response.code === 0) {
                        dispatch(showMessage({ message: "Xóa Loại thuốc/vật tư thành công" }));
                        refetchData();
                    } else {
                        dispatch(showMessage({ message: response.message}));
                    }
              ...
              <div className="flex flex-1 w-full items-center justify-between el-HeaderPageCustom">
                <div className="flex flex-col">
                  <div className="flex items-center mb-4">
                    ...
                    <Icon className="text-16" color="action">chevron_right</Icon>
                    <Typography color="textSecondary">Loại thuốc/vật tư</Typography>
                  </div>
                  <FuseAnimate>
                    <Typography variant="h6">{data.records} Loại thuốc/vật tư</Typography>
                  </FuseAnimate>
                </div>
                <DemoFilter
            ...
            content={
              <ReactTable
                ...
                columns={[
                  ...
                  {
                    Header: 'Mã Loại thuốc/vật tư',
                    accessor: 'code',
                    className: "wordwrap",
                    width: 120,
                  },
                  {
                    Header: 'Tên Loại thuốc/vật tư',
                    accessor: 'name',
                    className: "wordwrap",
                    maxWidth: 200
                  },
