...
function AppointmentDialog({ patientCode, _id, appointmentId, onSuccess, ...props }) {
  ...
  const handleClose = () => {
    ...
    dispatch(Actions.set_indication_in_store([]))
  };
          ...
          <Grid item md={4}>
            <Typography className="pl-12 text-15 font-bold mb-10 block-tittle">Thông tin đặt khám</Typography>
            {
              ...
              <div className="flex flex-wrap">
                ...
                <Grid container >
                  Địa chỉ: {appointment.patient? appointment.patient.street : ""} - {appointment.patient.ward ? appointment.patient.ward.name : ''} - {appointment.patient.district ? appointment.patient.district.name : ""} - {appointment.patient.nationality ? appointment.patient.nationality.name : ""}
                </Grid>
                <Divider />
