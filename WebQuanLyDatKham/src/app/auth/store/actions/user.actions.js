...
export function loadUserData() {
    ...
    graphqlService.query(ME).then(response => {
        ...
        if (history.location.pathname.startsWith('/login')) {
            ...
            if (regex.test(history.location.search)) {
                ...
                if (redirect.length > 1 && !redirect[1].startsWith("/login")) {
                  ...
                  window.location.href = "/apps/welcome"
        ...
        // history.push("/");
    }, (error) => store.dispatch(logoutUser())
    )
}
...
export function logoutUser() {
    ...
    return (dispatch, getState) => {
        ...
        // code demo thị trường
        // if (!pathname.startsWith("/login") && pathname !== "/choose-hospital" && pathname !== null && pathname !== "" && pathname !=="/") {
        //     history.push({
        //         pathname: '/login'
        //     });
        // }
