...
import { toggleCurrentCallPanel } from 'app/store/actions';
...
function CurrentCallPanel(props) {
    ...
    const display = useSelector(({ currentCallReducers }) => currentCallReducers.display);
    ...
    useEffect(() => {
        if (newCall && newCall.state === "RING") {
            dispatch(toggleCurrentCallPanel(true));
            // setExpandBody(true);
        ...
        display === true && newCall ?
            <Card className={classes.card}>
              <CardHeader
                ...
                action={
                  <IconButton aria-label="settings" onClick={e => dispatch(toggleCurrentCallPanel(false))}>
                    <Icon>close</Icon>
                  </IconButton>
                }
                ...
                <React.Fragment className = "el-CallPopUp">
                  {newCall.user&&<CardContent>
                    <div className="flex">
                      ...
                      <Typography>{newCall.user.gender === "1" ? "Nam" : "Nữ"}</Typography>
                    </div>
