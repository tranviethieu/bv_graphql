...
function CallsEvent(props) {
    ...
    const handleClickOpen = (data) => {
      if (data){
        dispatch(showAppointmentDialog({ data: _.omit(data,["_id","address", "avatar", "createdTime"])}));
      }
      else{
        dispatch(showAppointmentDialog({data: {phoneNumber: selectedCall.phoneNumber}}))
      }
    };
                ...
                <React.Fragment>
                  <AppBar className="w-full" position="static" elevation={1}>
                    <Toolbar className="px-16 flex flex-1 justify-between">
                      ...
                      <div>
                        <Button className='btn-blue' variant="contained" onClick={() => handleClickOpen(selectedCall.user)}>Tạo lịch khám</Button>{" "}
                      </div>
                    </Toolbar>
                  </AppBar>
