...
function Chat(props) {
  ...
  const chatReducer = useSelector(({ chatting }) => chatting);
  ...
  //trongpv
  const chatMessages = useSelector(({ chatApp }) => chatApp.chat.chat_messages);
  ...
  const chatPage = useSelector(({ chatApp }) => chatApp.chat.chat_page);
  const chatPageSize = useSelector(({ chatApp }) => chatApp.chat.chat_pageSize);
    ...
    if (handleMessage && handleMessage !== null) {
      if (handleMessage.type === selectedConversation.recent_message.type) {
        if (handleMessage.uid === selectedConversation.uid) {
          var tempmess = [...chatMessages, handleMessage]
          dispatch(Actions.setConversationMessages(tempmess))
        }
      }
    }
    // }, [chatMessages, dispatch, handleMessage, selectedConversation.recent_message.type, selectedConversation.uid]);
  }, [handleMessage]);
  ...
  useEffect(() => {
    console.log("chat reducer: ", chatReducer)
  }, [chatReducer])

  async function loadChatMessages(page, pageSize, conversationId) {
    if (conversationId) {
      canLoadMore = false
      setLoadMore(false)
      await Actions.getConversationMessages(page, pageSize, conversationId, dispatch)
        .then(response => {
          if (response && response.data) {
            console.log("===>response count: ", response.data.length)
            var messages = response.data.concat(chatMessages)
            dispatch(Actions.setConversationMessages(messages))

            console.log("===> length: ", response.data.length, "page size: ", pageSize)
  ...
  useMemo(() => {
    console.log("===> chay vao load more roi nhe, loadMore: ", loadMore, "can load more: ", canLoadMore);
    if (loadMore === true) {
      if (canLoadMore === true) {
        console.log("can load more true");
        var page = chatPage + 1
        dispatch(Actions.setChatPage(page))
      }
    }
  }, [canLoadMore, chatPage, dispatch, loadMore]);

  useMemo(() => {
    if (selectedConversation._id) {
      console.log("===> chat page: ", chatPage)
      loadChatMessages(chatPage, chatPageSize, selectedConversation._id)
    }

  }, [chatPage, chatPageSize, loadChatMessages, selectedConversation._id])
  ...
  function createMessage(mess, attachments, mesType = "TEXT") {
    ...
    var tempmess = [...chatMessages, messTemp]
    setMessageText('')
    dispatch(Actions.setConversationMessages(tempmess))
  ...
  return (
    <div className={clsx("flex flex-col relative", props.className)} style={{ minWidth: '130px' }} id="el-Chat-Cover">
      {/* {selectedConversation &&
            <Button variant="outlined" color="secondary" className={classes.button} onClick={handleLoadOldMessages}>
            Load more
            </Button>
          } */}
      <FuseScrollbars
      ...
      >
        {useMemo(() => (
          selectedConversation && chatMessages && chatMessages.length > 0 ?
            (
        ...
        ), [chatMessages, classes.messageRow, isFirstMessageOfGroup, isLastMessageOfGroup, selectedConversation, shouldShowContactAvatar])}
      </FuseScrollbars>
      {selectedConversation && (
                ...
                children: (
                  <React.Fragment>
                    ...
                    <DialogContent style={{ width: "auto !important" }}>
                      <UploadFileForm fileTypes={mediaType} server={process.env.REACT_APP_UPLOAD_URL} allowMultiple={true} maxFiles={1000} style={{ minHeight: "320px", marginBottom: "20px", }}
                        onSuccess={(files) => {
                          ...
                          //window.env.FILE_PREVIEW_URL + serverId
                        }}
                      >
                      </UploadFileForm>
                    </DialogContent>
