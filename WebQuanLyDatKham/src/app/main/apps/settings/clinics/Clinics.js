...
import { clinicsAttributes } from './components/ClinicsAttributes';
    ...
    return (dispatch) => dispatch(showQuickEditDialog(
        {
            ...
            subtitle: "Phòng",
            attributes: attributes || clinicsAttributes,
            data: data,
            submit: submit
...
function Clinics(props) {
    ...
    const [attributes, setAttributes] = useState(clinicsAttributes);
    ...
    function refetchData() {
        ...
        Actions.get_clinics(merged, dispatch).then(response => {
            setData(response);
        })
    }
    useEffect(() => {
    ...
    useEffect(() => {
        Actions.get_departments(dispatch).then(response => {
            setDepartments(response.data.map(g => ({
    ...
    useEffect(() => {
        const groupAtt = clinicsAttributes.find(a => a.name === "departmentCode");
        groupAtt.options = departments;
        setAttributes(clinicsAttributes);
    }, [departments]);

    function handleSave(form) {
        Actions.save_clinic(form, dispatch).then(response => {
            if (response.code === 0) {
    ...
    function handleDelete(data) {
        dispatch(showConfirmDialog({
            title: "Xóa Phòng", message: `Bạn có chắc muốn xóa Phòng ${data.name}?`, onSubmit: () => {
                Actions.remove_clinic(data._id, dispatch).then(response => {
                    if (response.code === 0) {
                        dispatch(showMessage({ message: "Xóa Phòng thành công" }));
                        refetchData();
                    } else {
                        dispatch(showMessage({ message: response.message}));
                    }
              ...
              <div className="flex flex-1 w-full items-center justify-between el-HeaderPage">
                <div className="flex flex-col">
                  <div className="flex items-center mb-4">
                    ...
                    <Icon className="text-16 el-TitleIcon" color="action">chevron_right</Icon>
                    <Typography color="textSecondary">Phòng</Typography>
                  </div>
                  <FuseAnimate>
                    <Typography variant="h6">{data.records} Phòng</Typography>
                  </FuseAnimate>
                </div>
                <DemoFilter
                  searchOption={{ onTextSearch, hideButton: true }}
                  filterOption={{
                    attributes: attributes.filter(item => item.name === "departmentCode").map(item => ({
            ...
            content={
              <ReactTable
                ...
                columns={[
                  ...
                  {
                    Header: 'Cho phép đăng ký khám',
                    accessor: 'allowAppointment',
                    className: "wordwrap",
                    width: 200,
                    Cell: row => <div>
                      {row.value === true ? "Có" : "Không"}
                    </div>
                  },
                ]}
              />
