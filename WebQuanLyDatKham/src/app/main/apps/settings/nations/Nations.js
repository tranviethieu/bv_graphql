...
import { nationalsAttributes } from './components/NationalsAttributes';
    ...
    return (dispatch) => dispatch(showQuickEditDialog(
        {
            ...
            subtitle: "Dân tộc",
            attributes: attributes || nationalsAttributes,
            data: data,
            submit: submit
...
function Nations(props) {
    ...
    const [attributes, setAttributes] = useState(nationalsAttributes);
    ...
    function refetchData() {
        ...
        Actions.get_nations(merged, dispatch).then(response => {
            setData(response);
        })
    }
    useEffect(() => {
    ...
    function handleSave(form) {
        Actions.save_nation(form, dispatch).then(response => {
            if (response.code === 0) {
    ...
    function handleDelete(data) {
        dispatch(showConfirmDialog({
            title: "Xóa dân tộc", message: `Bạn có chắc muốn xóa dân tộc ${data.name}?`, onSubmit: () => {
                Actions.remove_nation(data._id, dispatch).then(response => {
                    if (response.code === 0) {
                        dispatch(showMessage({ message: "Xóa dân tộc thành công" }));
                        refetchData();
                    } else {
                        dispatch(showMessage({ message: response.message}));
                    }
              ...
              <div className="flex flex-1 w-full items-center justify-between el-HeaderPage">
                <div className="flex flex-col">
                  <div className="flex items-center mb-4">
                    ...
                    <Icon className="text-16 el-TitleIcon" color="action">chevron_right</Icon>
                    <Typography color="textSecondary">Dân tộc</Typography>
                  </div>
                  <FuseAnimate>
                    <Typography variant="h6">{data? data.records : 0} Dân tộc</Typography>
                  </FuseAnimate>
                </div>
                <DemoFilter
