...
import { patientCategoriesAttributes } from './components/PatientCategoriesAttributes';
    ...
    return (dispatch) => dispatch(showQuickEditDialog(
        {
            ...
            subtitle: "Đối tượng",
            attributes: attributes || patientCategoriesAttributes,
            data: data,
            submit: submit
...
function PatientCategories(props) {
    const dispatch = useDispatch();
    const [data, setData] = useState({});
    const [attributes, setAttributes] = useState(patientCategoriesAttributes);
    ...
    function refetchData() {
        ...
        Actions.get_patientcategories(merged, dispatch).then(response => {
            setData(response);
        })
    }
    useEffect(()=>{
    ...
    function handleSave(form) {
        Actions.save_patientcategory(form, dispatch).then(response => {
            if (response.code === 0) {
    ...
    function handleDelete(data) {
        dispatch(showConfirmDialog({
            title: "Xóa Đối tượng", message: `Bạn có chắc muốn xóa Đối tượng ${data.name}?`, onSubmit: () => {
                Actions.remove_patientcategory(data._id, dispatch).then(response => {
                    if (response.code === 0) {
                        dispatch(showMessage({ message: "Xóa Đối tượng thành công" }));
                        refetchData();
                    } else {
                        dispatch(showMessage({ message: response.message}));
                    }
    ...
    useEffect(() => {
        const groupAtt = patientCategoriesAttributes.find(a => a.name === "type");
        groupAtt.options = patientType;
        setAttributes(patientCategoriesAttributes);
    }, [patientType]);
              ...
              <div className="flex flex-1 w-full items-center justify-between el-HeaderPage">
                <div className="flex flex-col">
                  <div className="flex items-center mb-4">
                    ...
                    <Icon className="text-16 el-TitleIcon" color="action">chevron_right</Icon>
                    <Typography color="textSecondary">Đối tượng</Typography>
                  </div>
                  <FuseAnimate>
                    <Typography variant="h6">{data.data? data.data.length : 0} Đối tượng</Typography>
                  </FuseAnimate>
                </div>
                <DemoFilter
