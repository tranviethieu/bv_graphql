...
import { organizationAttributes } from './components';
...
function PersonalOrganizationChart() {
  const dispatch = useDispatch();
  const [nodes, setNodes] = useState([])
  const [attributes, setAttributes] = useState(organizationAttributes);
  ...
  function handleSave(form) {
    ...
    Actions.savePersonel(submitData, dispatch).then(response => {
      if (response.code === 0) {
  ...
  function convertFormToData(form) {
    return _.omit(form, ["children", "_children", "collapse", "fullName", "parent", 'leader']);
  }
  ...
  function refetchData() {
    Actions.getPersonelsGraph().then(response => {
      if (response.code === 0) {
  ...
  useEffect(() => {
    refetchData();
    Actions.getAccounts(dispatch).then(response => {
      setAccounts(response.data.map(a => ({
        value: a._id, label: `${a.base.title} ${a.base.fullName} - ${a.department && a.department.name}`
      })))
    })
  }, [])
  ...
  useEffect(() => {
    ...
    const leaderAttribute = organizationAttributes.find(m => m.name == "leaderId");
    leaderAttribute.options = accounts;
    const parentAttribute = organizationAttributes.find(m => m.name == "parentId");
    parentAttribute.options = menuList.map(m => ({
      value: m._id, label: m.fullName
    }))
    setAttributes([...organizationAttributes])
  }, [accounts, menuList]);
  ...
  const MyNodeComponent = ({ node }) => {
    ...
    function handleDelete(_id) {
      Actions.removePersonel(_id).then(response => {
        if (response.code === 0) {
    ...
    return (
      <div className="flex flex-col items-center">
        <Card className={classes.root}>
          <CardHeader title={<Tooltip title={node.description}>
            <Typography className="font-bold text-14" aria-controls="simple-menu" aria-haspopup="true" onClick={handleClick}>{node.name}</Typography>
          </Tooltip>}
            action={
          ...
          {
            (node.children.length > 0 || (node._children && node._children.length > 0)) &&
        ...
        <div className="flex flex-1 w-full items-center justify-between el-HeaderPage">
          ...
          <div className="flex items-center">
            ...
            <FuseAnimate animation="transition.slideLeftIn" delay={300}>
              <Typography className="hidden sm:flex" variant="h6">Sơ đồ tổ chức hành chính</Typography>
            </FuseAnimate>
          </div>
...
export default (PersonalOrganizationChart);
