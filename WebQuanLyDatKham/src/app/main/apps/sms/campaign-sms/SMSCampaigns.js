...
function SMSCampaigns(props) {
    ...
    const [tableFiltered, setTableFiltered] = useState({ page: 0, pageSize: 10, sorted: [{id:"priority",desc:false}] })

    function onTextSearch(text) {
      setTextFilter({ id: "name", value: text.replace(/[^0-9a-zàáạảãâầấậẩẫăằắặẳẵèéẹ&ẻẽêềếệểễìí!ịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđ,-/.\s]/gi, '') });
        // setSearchText(text.replace(/[^0-9a-zàáạảãâầấậẩẫăằắặẳẵèéẹ&ẻẽêềếệểễìí!ịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđ,-/.\s]/gi, ''));
    }
    function fetchData() {
        const merged = { ...tableFiltered, filtered: [...formFiltered, textFilter] }
        getSMSCampaigns(merged, dispatch).then(
            response => {
                setData(response);
            }
        )
    ...
    return (
        <FusePageCarded
          ...
          header={
            <div className="flex flex-1 w-full items-center justify-between el-HeaderPage" id = "el-SMSCampaigns-Header">
              ...
              <div className="flex items-center">
                ...
                <FuseAnimate animation="transition.slideLeftIn" delay={300}>
                  <Typography className="hidden sm:flex" variant="h6">Chiến dịch gửi SMS ({data.records})</Typography>
                </FuseAnimate>
              </div>
              <DemoFilter
                searchOption={{ onTextSearch, hideButton: true }}
                createOption={{ onClick: () => history.push("/apps/sms/campaign-sms/new") }}
              />
