...
import InfoTest from '../Dialog/InfoTest';
...
function TestResults(props){
    ...
    const [testResults, setTest] = useState([])
    ...
    const onChangeTable = (state) => {
      ...
      filtered.forEach(function(item, index){
          if(item.id === "user.FullName" || item.id === "user.PhoneNumber" || item.id === "Data.Title" || item.id === "Data.TestId") {
    ...
    const handleClickOpen = (type) => {
        dispatch(showUserActionDialog({rootClass: classes.root,  phoneNumber: "", type: type, className: "el-coverUAD-TestResult" }))
            ...
            <FusePageCarded
                ...
                <div className="flex flex-1 w-full items-center justify-between el-HeaderPage">
                  <div className="flex items-center">
                    ...
                    <FuseAnimate animation="transition.slideLeftIn" delay={300}>
                      <Typography className="hidden sm:flex" variant="h6">Kết quả xét nghiệm ({testResults.records? testResults.records : 0})</Typography>
                    </FuseAnimate>
                  </div>
                  <FuseAnimate animation="transition.slideRightIn" delay={300}>
                    <Button onClick = {()=>handleClickOpen("TESTRESULT")} className="whitespace-no-wrap btn-blue" variant="contained">
                      <span className="hidden sm:flex">Tạo kết quả xét nghiệm</span>
                    </Button>
                  </FuseAnimate>
                </div>
              }
              content={
                <div className = "el-cover-table">
                  <InfoTest onCloseInfoDialog = {handleCloseInfo} open = {open} test = {info} user={form}/>
                  {openLightBox && <Lightbox
                    className = "el-Lightbox"
                    imageTitle={"Kết quả xét nghiệm " + form.name + " vào "+ moment(form.date).format("HH:mm DD/MM/YYYY")}
                  ...
                  <ReactTable
                    ...
                    columns={[
                      ...
                      {
                        Header: "Các xét nghiệm",
                        accessor: "data.UnitResults",
                        filterable: false,
                        Cell: row =>
                        <div>
                          {
                            row.value && row.value.length>0?
                              <label htmlFor="text-button-file">
                                <Button
                                  ...
                                  component="span"
                                  onClick = {() =>{setOpen(true); setInfo(row.value); setForm({ currenImages: row.value, name: row.original.user.fullName, date: row.original.createdTime })}}>{row.value.length} xét nghiệm
                                </Button>
                              </label>
                            : <label htmlFor="text-button-file">
                              <Button
                        ...
                        <div>
                          {
                            row.value.length>0?
                              <label htmlFor="text-button-file">
                                <Button className = "el-ButtonLowerCase-ShowDialog" component="span"
