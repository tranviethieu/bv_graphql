...
function Survey(props) {
    ...
    useEffect(() => {
        ...
        if (_id) {
            ...
            let type = "USER"
            Actions.getSurveysGeneralReport(mbegin, mend, [_id], type)
                .then(response => {
                    console.log("===> report respones: ", response)
                    setData(response.data)
                })
            Actions.getSurveyFrequenceData(mbegin, mend, [_id], 10, type, dispatch)
                .then(response => {
    ...
    function fetchUsersData(state) {
        ...
        if (_id) {
            ...
            Actions.getUsersBySurvey(mbegin, mend, page, pageSize, filtered, sorted, surveyIds, dispatch).then(response => {
    ...
    async function getQuestionData(questions) {
        ...
        if (ids) {
            ...
            let mend = moment(form.end).format("YYYY-MM-DD")
            let type = "USER"
            await Actions.getQuestionsAnalysticData(ids, mbegin, mend, type)
                .then(response => {
    ...
    return (
        <div>
            <AnswerPreview open={open}
                ...
                user={lastAction && { phoneNumber: lastAction.phoneNumber, fullName: lastAction.user ? lastAction.user.fullName : '' }}
                result={lastAction && lastAction.data && lastAction.data}
                title={lastAction && lastAction.survey.title && lastAction.survey.title} />
            <FusePageSimple
                classes={{
                            ...
                            <div className="p-12 flex flex-wrap">
                                {
                                    useMemo(() => {
                                        return (
                                            <div className="widget sm:w-5/12 md:w-5/12 l:w-5/12 p-12">
                                ...
                                <div className="widget sm:w-1/6 md:w-1/6 l:w-1/6 p-12" style={{ marginTop: "10px" }}>
                                    <FuseAnimate animation="transition.slideRightIn" delay={300}>
                                        <Button className="whitespace-no-wrap" variant="contained"
                                            onClick={e => {
                                                let begin = moment(form.begin).format("YYYY-MM-DD")
                                                let end = moment(form.end).format("YYYY-MM-DD")
                                                const _id = props.match.params._id;
                                                let url = `https://api.app.benhvienphusanhanoi.vn/app-noi-bo/survey/export/survey_result?begin=${begin}&end=${end}&surveyIds=${_id}`
                                                console.log("url: ", url)
                                                window.location.href = url
                                            }}>
                                            <span className="hidden sm:flex">Xuất file</span>
                                        </Button>
                                    </FuseAnimate>
                                </div>
                            </div>
                                                ...
                                                questionCharts && questionCharts.map(item => {
                                                    ...
                                                    return (
                                                        ...
                                                        <div className="widget sm:w-1/3 md:w-1/3 l:w-1/3 p-12">
                                                            {/* <div className="widget sm:w-full md:w-full l:w-full p-12"> */}
                                ...
                                <ReactTable
                                    ...
                                    columns={[
                                        ...
                                        },
                                        {
                                            ...
                                            accessor: 'same_user_answer_numb',
                                            filterable: false,
