...
import {Button, Dialog, DialogActions, DialogContent, DialogContentText, Grid, Step, StepButton, Stepper, Icon, AppBar, Typography, CircularProgress} from '@material-ui/core';
...
function getSteps (){
  return ['Chưa tiếp nhận', 'Đang thực hiện']
}
function UpdateJobToMeDialog(props) {
    ...
    useEffect(()=>{
      if(percent === 100){
        ...
        setSteps(['Đang thực hiện', 'Đã hoàn thành'])
      }
    },[percent])
    const handleStep = step => () => {
      setActiveStep(step);
      if (jobDetail.state === "ASSIGN"){
          if(step === 0){
    ...
    function fetchData(){
      ...
      Actions.getJobDetail(data._id,dispatch).then(response =>{
        ...
        if(response.data.state === "ASSIGN"){
          setActiveStep(0);
          setSelectedValue("ASSIGN");
          setSteps(getSteps())
        }
        else if (response.data.state === "PROCESSING" && (response.data.process === 100 || percent === 100)){
          setActiveStep(0);
          setSelectedValue("PROCESSING")
          setSteps(['Đang thực hiện', 'Đã hoàn thành'])
        }
    ...
    function handleSubmit(){
      if(selectedValue!== jobDetail.state || percent!== jobDetail.process){
        setLoading(true)
  ...
  return (
      <Dialog open={open} onEnter={fetchData} onClose={onCloseDialog} aria-labelledby="form-dialog-title" classes={{paper: "w-full m-24 rounded-8"}}>
              ...
              </Grid>
              {
                ...
                : <Grid container spacing={1} className = 'p-4'>
                  ...
                  <Grid item xs={6}>
                    <div className = "font-bold el-GridWordWrap">
                      {
                        jobDetail.state === "ASSIGN" || jobDetail.state === "COMPLETE" || jobDetail.state === "CANCEL" ?
                          jobDetail.process + "%"
                        : <div>
                          <div>
                            {percent + "%"}
                  ...
                  {
                    jobDetail.state === "ASSIGN" || (jobDetail.state === "PROCESSING" && (percent === 100 || jobDetail.process === 100)) ?
                    ...
                    : <Grid container spacing={1} className = 'w-full p-4'>
                      <Grid item xs={6}>
                        Trạng thái:
                      </Grid>
                      <Grid item xs={6}>
        ...
        {
          data.showType === 'info' || jobDetail.state === "CANCEL" || jobDetail.state === "COMPLETE"?
            <DialogActions>
...
export default UpdateJobToMeDialog;
