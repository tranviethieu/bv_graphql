...
import { Paper, Button, Input, Icon, Typography, Menu, MenuItem, IconButton, Tooltip, FormControlLabel, TextField } from '@material-ui/core';
import { FuseAnimate, FuseChipSelect, FusePageSimple, AdvanceEditor, FuseAnimateGroup } from '@fuse';
...
function NotifySend(props) {
    ...
    function fetchData(state) {
        if (state) {
            ...
            Actions.getNotificationSends({ page, pageSize, filtered: filter, sorted: tempSort }, dispatch).then(response => {
            ...
            Actions.getNotificationSends({ page, pageSize, filtered: filter, sorted: sorted }, dispatch).then(response => {
                console.log("===> notifi queues: ", response)
                setNotifies(response);
                setLoading(false)
            })
    ...
    function handleDelete(data) {
        dispatch(showConfirmDialog({
            title: "Xóa thông báo", message: `Bạn có chắc muốn xóa thông báo ${data.title} ?`, onSubmit: () => {
                Actions.removeNotificationSend(data._id, dispatch).then(response => {
                    if (response.code === 0) {
                        dispatch(showMessage({ message: "Xóa thông báo thành công" }));
                        fetchData(null);
                    } else {
                ...
                header={
                    <div className="flex flex-1 items-center justify-between p-24 el-HeaderPage">
                        <div className="flex flex-col">
                            <Typography variant="h6">Báo cáo thông báo đã gửi</Typography>
                        </div>
                    </div>
                }
                content={
