...
import { medicalCategoriesAttributes } from './components/MedicalCategoriesAttributes';
    ...
    return (dispatch) => dispatch(showQuickEditDialog(
        {
            ...
            attributes: attributes || medicalCategoriesAttributes,
            data: data,
            submit: submit
...
function MedicalCategories(props) {
    ...
    const [attributes, setAttributes] = useState(medicalCategoriesAttributes);
    ...
    function refetchData() {
        ...
        Actions.get_medical_categories(merged, dispatch).then(response => {
            setData(response);
        })
    }
    useEffect(() => {
    ...
    function handleSave(form) {
        Actions.save_medical_category(form, dispatch).then(response => {
            if (response.code === 0) {
    ...
    function handleDelete(data) {
        dispatch(showConfirmDialog({
            title: "Xóa Loại dịch vụ", message: `Bạn có chắc muốn xóa Loại dịch vụ ${data.name}?`, onSubmit: () => {
                Actions.remove_medical_category(data._id, dispatch).then(response => {
                    if (response.code === 0) {
                        dispatch(showMessage({ message: "Xóa Loại dịch vụ thành công" }));
                        refetchData();
                    } else {
                        dispatch(showMessage({ message: response.message}));
                    }
              ...
              <div className="flex flex-1 w-full items-center justify-between el-HeaderPageCustom">
                <div className="flex flex-col">
                  <div className="flex items-center mb-4">
                    ...
                    <Icon className="text-16" color="action">chevron_right</Icon>
                    <Typography color="textSecondary">Loại dịch vụ</Typography>
                  </div>
                  <FuseAnimate>
                    <Typography variant="h6">{data.records} Loại dịch vụ</Typography>
                  </FuseAnimate>
                </div>
                <DemoFilter
