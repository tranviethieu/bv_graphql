...
import { worksAttributes } from './components/WorksAttributes';
    ...
    return (dispatch) => dispatch(showQuickEditDialog(
        {
            ...
            attributes: attributes || worksAttributes,
            data: data,
            submit: submit
...
function Works(props) {
    ...
    const [attributes, setAttributes] = useState(worksAttributes);
    ...
    function refetchData() {
        ...
        Actions.get_works(merged, dispatch).then(response => {
            setData(response);
        })
    }
    useEffect(() => {
    ...
    function handleSave(form) {
        Actions.save_work(form, dispatch).then(response => {
            if (response.code === 0) {
    ...
    function handleDelete(data) {
        dispatch(showConfirmDialog({
            title: "Xóa nghề nghiệp", message: `Bạn có chắc muốn xóa nghề nghiệp ${data.name}?`, onSubmit: () => {
                Actions.remove_work(data._id, dispatch).then(response => {
                    if (response.code === 0) {
                        dispatch(showMessage({ message: "Xóa nghề nghiệp thành công" }));
                        refetchData();
                    } else {
                        dispatch(showMessage({ message: response.message}));
                    }
              ...
              <div className="flex flex-1 w-full items-center justify-between el-HeaderPageCustom">
                <div className="flex flex-col">
                  <div className="flex items-center mb-4">
                    ...
                    <Icon className="text-16" color="action">chevron_right</Icon>
                    <Typography color="textSecondary">Nghề nghiệp</Typography>
                  </div>
                  <FuseAnimate>
                    <Typography variant="h6">{data.records} Nghề nghiệp</Typography>
                  </FuseAnimate>
                </div>
                <DemoFilter
            ...
            content={
              <ReactTable
                ...
                columns={[
                  ...
                  {
                    Header: 'Mã nghề nghiệp',
                    accessor: 'code',
                    className: "wordwrap",
                    width: 120,
                  },
                  {
                    Header: 'Tên nghề nghiệp',
                    accessor: 'name',
                    className: "wordwrap",
                    maxWidth: 200
                  },
