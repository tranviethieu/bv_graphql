...
const wsLink = new WebSocketLink({
  uri: process.env.REACT_APP_API_WS_HOST,
  options: {
    reconnect: true
  }
});
...
//hàm này trả về đối tượng dạng Observable
export const watchQuery = (query, variables, dispatch) => {
  var request = client.watchQuery({
    query,
    variables
  });
  return request.then(result => elsagaService.handleResponse(result, dispatch))
